  ...
  useEffect(() => {
    if (pathRef.current) {
      ...
      if (length > 30) {
        ...
        const pt2_next = pathRef.current.getPointAtLength(Math.min(dist2 + 2, length));
  ...
  return (
    <>
      <BaseEdge 
        path={edgePath} 
        markerEnd={markerEnd} 
        style={{ ...style, stroke: selected ? '#FF5555' : '#4A3C32', strokeWidth: selected ? 2 : 1, strokeDasharray: '4 4', opacity: selected ? 1 : 0.8 }} 
      />
