...
const TUTORIAL_ITEMS = [
    { target: '.ws-icon--door', text: 'Click the door to change themes!', position: 'left' },
    { target: '.ws-icon--sketchbook', text: 'Open the sketchbook to draw!', position: 'left' },
    { target: '.ws-icon--laptop', text: 'Download your flowchart!', position: 'left' },
...
const TutorialOverlay = ({ onDismiss }) => {
    ...
    return ReactDOM.createPortal(
        <div style={{ position: 'fixed', inset: 0, zIndex: 100000 }}>
            ...
            <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.55)', pointerEvents: 'auto' }} onClick={handleNext} />
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '8px' }}>
                    <button onClick={handleSkip} style={{
                        ...
                        fontFamily: 'var(--font-mono)', fontSize: '0.65rem', cursor: 'pointer', textTransform: 'uppercase'
                    }}>Skip</button>
                    <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.6rem', color: 'rgba(255,255,255,0.4)' }}>
                        {step + 1}/{TUTORIAL_ITEMS.length}
                    </div>
                    <button onClick={handleNext} style={{
                        background: '#FFFFFF', color: '#0D0D0D', border: 'none',
                        fontFamily: "'OMORI_GAME', cursive", fontSize: '0.85rem', padding: '3px 12px', cursor: 'pointer'
                    }}>{step >= TUTORIAL_ITEMS.length - 1 ? 'Done' : 'Next'}</button>
                </div>
            </div>
        </div>,
        document.body
...
const THEME_LIST = [
    ...
    { value: 'neo-brutalist', label: 'NEO-BRUTALIST', desc: 'Bold, loud, unapologetic' },
    { value: 'editorial', label: 'EDITORIAL', desc: 'Clean serif minimalism' },
    { value: 'corporate', label: 'CORPORATE', desc: 'Sleek dark professionalism' },
...
const DoorThemePickerModal = ({ currentTheme, onSelect, onClose }) => {
    return ReactDOM.createPortal(
        <div className="omori-modal-overlay" style={{ zIndex: 99999 }} onClick={onClose}>
            <div
            ...
            >
                <div style={{ fontSize: '0.65rem', fontFamily: 'var(--font-mono)', letterSpacing: '0.15em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.5)', marginBottom: '4px' }}>
                    THROUGH THE DOOR...
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', overflowY: 'auto' }}>
                    {THEME_LIST.map((t) => (
                        <button
                            ...
                            style={{
                                background: currentTheme === t.value ? '#FFFFFF' : 'transparent',
                            ...
                            onMouseOver={(e) => { if (currentTheme !== t.value) { e.currentTarget.style.background = 'rgba(255,255,255,0.1)'; } }}
                            onMouseOut={(e) => { if (currentTheme !== t.value) { e.currentTarget.style.background = 'transparent'; } }}
                        >
                            <span>{t.label}</span>
                            <span style={{ fontSize: '0.6rem', fontFamily: 'var(--font-mono)', color: currentTheme === t.value ? '#666' : 'rgba(255,255,255,0.4)', fontStyle: 'normal' }}>{t.desc}</span>
                        </button>
                    ))}
                </div>
                <button onClick={onClose} style={{
                    marginTop: '14px', background: 'transparent', border: '2px solid rgba(255,255,255,0.3)',
                    color: 'rgba(255,255,255,0.5)', padding: '6px 16px', cursor: 'pointer',
                    fontFamily: "'OMORI_GAME', cursive", fontSize: '0.85rem', alignSelf: 'center',
                    transition: 'all 0.15s',
                }}
                    onMouseOver={(e) => { e.currentTarget.style.borderColor = '#FFFFFF'; e.currentTarget.style.color = '#FFFFFF'; }}
                    onMouseOut={(e) => { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.3)'; e.currentTarget.style.color = 'rgba(255,255,255,0.5)'; }}
...
// box
const WhiteSpaceBox = ({ isDarkMode, onClear, onDownload, onDrawToggle, onDoorOpen }) => {
...
// nav bar
const NavToolbar = ({ handlePreviewDownload, isDarkMode, toggleTheme, onClearAll }) => {
...
export const PipelineUI = () => {
    ...
    useEffect(() => {
        document.body.classList.remove('theme-omori', 'theme-neo-brutalist', 'theme-editorial', 'theme-corporate', 'theme-playful', 'theme-wireframe', 'theme-medical', 'darkMode');
    ...
    const handlePreviewDownload = async () => {
        ...
        if (!flowEl) { setIsCapturing(false); return; }
        ...
        setTimeout(async () => {
            try {
                ...
                const [pngDataUrl, svgDataUrl] = await Promise.all([
                    toPng(flowEl, { backgroundColor: bgColor, pixelRatio: 2, ...filterOptions }),
    ...
    return (
        ...
        <select 
            ...
            style={{ position: 'fixed', top: '20px', left: '20px', zIndex: 10, border: '2px solid var(--ws-black)', background: 'var(--ws-white)', color: 'var(--ws-black)', width: 'auto', padding: '4px 12px', fontFamily: 'var(--font-mono)', fontSize: '0.8rem', fontWeight: 'bold', textTransform: 'uppercase', cursor: 'pointer' }}
            title="Select Theme"
        >
        ...
        {/* White Space box */}
        <WhiteSpaceBox isDarkMode={isDarkMode} onClear={() => setShowConfirm(true)} onDownload={handlePreviewDownload} onDrawToggle={() => setActiveTool("draw")} onDoorOpen={() => setShowDoorPicker(true)} />
        ...
        {/* Door Theme Picker */}
        {showDoorPicker && (
            <DoorThemePickerModal currentTheme={theme} onSelect={setTheme} onClose={() => setShowDoorPicker(false)} />
        )}
        ...
        {/* MEWO Loading Screen */}
        {isCapturing && ReactDOM.createPortal(
            <div style={{
        ...
        <div ref={reactFlowWrapper} className={activeTool === "erase" ? "erasing" : ""} style={{width: '100vw', height: '100vh', position: 'absolute', top: 0, left: 0, zIndex: 1}}>
            ...
            >
                ...
                />
                <NavToolbar handlePreviewDownload={handlePreviewDownload} isDarkMode={isDarkMode} toggleTheme={toggleTheme} onClearAll={() => setShowConfirm(true)} />
                <DrawingCanvas />
            </ReactFlow>
        </div>
        </>
