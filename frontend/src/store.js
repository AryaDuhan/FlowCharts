...
export const useStore = create((set, get) => ({
  ...
  toggleLock: () => {
    ...
    if (selectedNodes.length === 0 && selectedEdges.length === 0) {
      ...
      // toggle locks
      get().saveState();
      const anyUnlocked = [...selectedNodes, ...selectedEdges].some(
        ...
        nodes: nodes.map((n) =>
          n.selected
            ? {
                ...n,
                draggable: !lockState,
                data: { ...n.data, locked: lockState },
              }
            : n,
        ),
        edges: edges.map((e) =>
          e.selected ? { ...e, data: { ...e.data, locked: lockState } } : e,
        ),
      });
    }
  },
  ...
  onConnect: (connection) => {
    ...
    set({
      edges: addEdge(
        { ...connection, type: "hand", animated: false },
        get().edges,
      ),
    });
  },
